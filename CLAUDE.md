...
| `/implement-issues <ISSUE> [<ISSUE> ...]` | Space-separated list of GitHub issue references; each token validated independently against the same regex as `/implement-issue`. Whole batch rejected on any token mismatch before any `gh` invocation. Mixed formats allowed (e.g. `57 #58 https://github.com/andrekirst/strg/issues/60`). | Fetches all N issues, spawns a Pre-Flight Analyst (`Explore`) plus a long-lived Coordinator subagent (`general-purpose`, named `coordinator`) that dispatches N parallel pipelines — one per issue, each in its own git worktree on `multi-issue/<n>`, each opening its own PR. Tiered failure handling on Analyst-detected `depends_on` edges; two-tier batch close-gate. See `.claude/agents/multi-issue-team.md`. |
