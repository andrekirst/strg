...
/// (each <c>Drive</c> carries its own <c>ProviderType</c> + <c>ProviderConfig</c>), so the
/// writer cannot constructor-inject <c>IStorageProvider</c> via DI — there is no sensible
...
///
/// <para><b>Why this is a port, not inline construction.</b> <see cref="AesGcmFileWriter"/>
