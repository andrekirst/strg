...
///
/// <para><b>Threat model.</b> DEKs are per-file-version, written to the <c>file_keys</c> table
/// in wrapped form. An attacker with filesystem access gets only ciphertext + wrapped DEKs;
/// without the KEK they cannot unwrap the DEK and therefore cannot decrypt the payload. This
/// is the reason the KEK MUST NOT be co-located with the database (env var / mounted secret /
/// KMS — not a row).</para>
///
...
/// will require a <c>KeyId</c> column on the file-key row; the interface will grow a key-
/// version return value then. Callers should treat the wrapped-DEK byte array as opaque — the
/// wire layout is an implementation detail of the <see cref="IKeyProvider"/>.</para>
/// </summary>
